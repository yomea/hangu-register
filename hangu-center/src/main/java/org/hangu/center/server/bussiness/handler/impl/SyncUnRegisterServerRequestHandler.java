 ...
 */
public class SyncUnRegisterServerRequestHandler extends AbstractRequestHandler<List<RegistryInfo>> {

    private ServiceRegisterManager serviceRegisterManager;

    public SyncUnRegisterServerRequestHandler(ServiceRegisterManager serviceRegisterManager) {
        this.serviceRegisterManager = serviceRegisterManager;
    }

    @Override
    public CommandTypeMarkEnum support() {
        return CommandTypeMarkEnum.BATCH_SYNC_REMOVE_SERVICE;
    }
    ...
    public Response doHandler(Request<List<RegistryInfo>> request, NettyServer nettyServer) {
        ...
        registryInfos.stream().forEach(serviceRegisterManager::syncUnRegistry);
